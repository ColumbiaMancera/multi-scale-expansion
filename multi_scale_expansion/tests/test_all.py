...
def test_train_model(): # mock_dir, mock_transforms
    ...
    mock_step_size = 7
    mock_gamma = 0.1
    mock_criterion, mock_optimizer, mock_lr_scheduler = get_train_loss_needs(mock_model, 
                                                                             mock_lr, 
                                                                             mock_momentum, 
                                                                             mock_step_size, 
                                                                             mock_gamma)
    ...
    model, train_losses, train_accuracies, val_losses, val_accuracies = train_model(device, mock_dataset_sizes, mock_dataloaders, mock_model, mock_criterion, mock_optimizer, mock_lr_scheduler, num_epochs=1)
    assert isinstance(model, nn.Module)
